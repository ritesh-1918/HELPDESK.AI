...
1. **Unprotected Local Storage:** Early iterations lacked defensive error bounds around `localStorage` syncing, risking complete UI lockouts on quota exceptions. We learned to _always_ wrap browser storage APIs in fault-tolerant `try/catch` boundaries.
