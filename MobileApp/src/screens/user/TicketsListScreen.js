...
const TicketsListScreen = () => {
  ...
  const getStatusConfig = (status) => {
    switch (status) {
      case "resolved":
        return { color: COLORS.success, label: "RESOLVED", icon: CheckCircle2 };
      case "in_progress":
        return { color: "#3b82f6", label: "IN PROGRESS", icon: Clock };
      case "pending_human":
        return { color: "#f59e0b", label: "ESCALATED", icon: AlertTriangle };
      default:
  ...
  const renderTicket = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          navigation.navigate("TicketTracking", { ticketId: item.id })
        }
        activeOpacity={0.8}
      >
          ...
          >
            ...
            {f !== "All" && (
              <Text
                style={[
                  styles.chipCount,
                  activeFilter === f && styles.chipCountActive,
                ]}
              >
                {f === "Active"
