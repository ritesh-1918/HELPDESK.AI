      ...
      <View key={s} style={styles.stepItem}>
        ...
        >
          {currentStep > s ? (
            ...
            <Text
              style={[styles.stepNum, currentStep >= s && styles.stepNumActive]}
            >
              {s}
            </Text>
          )}
        ...
        {s < 3 && (
          <View
            style={[styles.stepLine, currentStep > s && styles.stepLineActive]}
          />
        )}
      </View>
    ))}
...
const ForgotPasswordScreen = () => {
  ...
  const handleVerifyOtp = async () => {
    if (!otp || otp.length !== 6) {
      notifyError("Invalid Code", "Please enter the 6-digit code.");
      return;
    }
    if (timerExpired) {
      notifyError("Code Expired", "Please request a new code.");
      return;
    }
    setLoading(true);
    try {
      ...
      notifyError(
        "Invalid Code",
        "The code is wrong or expired. Please try again.",
      );
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        >
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              step === 1 ? navigation.goBack() : goToStep(step - 1)
            }
          >
