...
const Toast = ({ id, type = "info", title, message, onDismiss }) => {
  ...
  return (
    ...
    >
      <View
        style={[styles.toastIcon, { backgroundColor: config.iconColor + "18" }]}
      >
      ...
      <View style={styles.toastContent}>
        ...
        {message ? (
          <Text style={[styles.toastMessage, { color: config.text + "cc" }]}>
            {message}
          </Text>
        ) : null}
      </View>
...
export const NotificationProvider = ({ children, topInset = 50 }) => {
  ...
  const success = useCallback(
    (title, message, opts) =>
      notify({ type: "success", title, message, ...opts }),
    [notify],
  );
  const error = useCallback(
    (title, message, opts) =>
      notify({ type: "error", title, message, ...opts }),
    [notify],
  );
  const warning = useCallback(
    (title, message, opts) =>
      notify({ type: "warning", title, message, ...opts }),
    [notify],
  );
