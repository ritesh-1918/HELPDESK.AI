            ...
            >
              <button
                ...
                style={{
                  ...
                  transition: 'all 0.2s',
                  ...(!isInternal
                    ? { background: '#0f1f12', color: '#ffffff' }
        ...
        {loading ? (
          ...
          grouped.map((item, i) => {
            ...
            return (
              <div
                key={msg.id || i}
                className={`flex gap-2.5 ${isMe ? 'justify-end' : 'justify-start'} group py-1`}
              >
                {!isMe && (
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-auto mb-1 shadow-sm
                ...
                >
                  <div className={`flex items-center gap-2 px-1`}>
                    <span
                      className={`text-[9px] font-black uppercase tracking-widest ${msg.is_internal ? 'text-amber-600' : 'text-slate-400'}`}
                    >
                      {msg.is_internal
                        ? '🔒 Internal Note'
                        : isMe
                          ? 'You'
                          : msg.sender_name || (isAdmin ? 'Support ' : 'User')}
                    </span>
