...
function Result() {
  ...
  const solutionSteps = Array.isArray(data.solution_steps)
    ...
    : typeof data.solution_steps === 'string'
      ...
      : [
          data.solution_steps ||
            'Consult internal knowledge base for specific troubleshooting steps.',
        ];
      ...
      <div className='max-w-4xl mx-auto space-y-6'>
        ...
        <Card>
          <div className='flex justify-between items-start'>
            <div>
              <h2 className='text-2xl font-bold mb-2'>Ticket Analysis Result</h2>
              <p className='text-gray-500'>
                Ticket ID:{' '}
                <span className='font-mono font-semibold'>{data.ticket_id || 'N/A'}</span>
              </p>
            </div>
          ...
          <div className='space-y-4'>
            ...
            <div>
              <div className='flex items-center justify-between mb-2'>
                ...
                <span className='text-gray-600 font-bold'>
                  {Math.round((data.routing_confidence || data.confidence || 0) * 100)}%
                </span>
              </div>
              <Progress
                percent={Math.round((data.routing_confidence || data.confidence || 0) * 100)}
                status={
                  (data.routing_confidence || data.confidence || 0) > 0.8 ? 'success' : 'normal'
                }
                strokeColor={{
                  '0%': '#108ee9',
                  '100%': '#87d068',
            ...
            {(data.summary || data.reasoning) && (
              <div className='space-y-4 pt-4 border-t border-gray-100'>
                {data.summary && (
                  <div>
                    <span className='font-semibold text-gray-400 text-xs uppercase tracking-widest block mb-2'>
                      AI Summary:
                    </span>
          ...
          >
            <div className='space-y-4'>
              {data.image_description && (
                <div>
                  <span className='font-semibold text-gray-400 text-xs uppercase tracking-widest block mb-1'>
                    Image Description:
                  </span>
                            ...
                            <div>
                              ...
                              {data.duplicate_probability > 0.7 && (
                                <p className='text-xs text-gray-500 mt-1 flex items-center gap-1'>
                                  <ClockCircleOutlined /> Consider checking ticket history before
                                  proceeding
                                </p>
                              )}
                            </div>
