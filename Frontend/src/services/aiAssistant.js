...
const buildConfigList = () => {
  ...
  const geminiModels = (
    env.VITE_AI_GEMINI_MODELS || 'gemini-2.5-flash,gemini-2.5-flash-lite,gemini-2.0-flash'
  ).split(',');
  ...
  const openrouterKeys = [
    ...
    env.VITE_OPENROUTER_API_KEY_2,
    env.VITE_OPENROUTER_API_KEY_3,
    env.VITE_OPENROUTER_API_KEY_4,
  ].filter(Boolean);
  const openrouterModels = (
    env.VITE_AI_OPENROUTER_MODELS ||
    'meta-llama/llama-3.2-3b-instruct:free,microsoft/phi-3-mini-128k-instruct:free,mistralai/mistral-7b-instruct:free,google/gemma-2-9b-it:free'
  ).split(',');
...
const runWithFailover = async (promptText, history, image) => {
  ...
  for (let i = 0; i < configList.length; i++) {
    ...
    if (blacklistedKeys.has(config.key)) {
      console.log(
        `[AI Failover] Skipping blacklisted key for ${config.provider} (${config.model})`
      );
      continue;
    }
...
User Issue: "${issueText}"${imageNote}${imageInstruction}`;
  ...
  try {
    ...
    console.warn(
      '[analyzeTicketWithAI] All providers exhausted, using local fallback:',
      err.message
    );
