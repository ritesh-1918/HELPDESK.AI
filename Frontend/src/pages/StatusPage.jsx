...
export default function StatusPage() {
  ...
  const services = [
    ...
    },
    {
      ...
      status: 'Operational',
      desc: 'Local Web Speech Recognition browser framework compatibility',
    },
    {
      name: 'Client-Side OCR Telemetry',
      status: 'Operational',
      desc: 'Tesseract.js script injection & parallel image worker processes',
    },
    {
        ...
        <div className='bg-emerald-950 text-white rounded-[2rem] p-8 md:p-10 shadow-2xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6'>
          ...
          <div className='space-y-3 z-10'>
            ...
            <p className='text-slate-300 text-xs font-semibold'>
              100% of microservices running successfully within target parameters.
            </p>
          </div>
        ...
        <div className='space-y-4'>
          ...
          <div className='space-y-3'>
            {services.map((service, idx) => (
              <Card
                key={idx}
                className='p-6 rounded-[1.5rem] border border-slate-200/80 bg-white flex items-center justify-between shadow-sm flex-wrap gap-4'
              >
