...
const categories = [
  {
    ...
    desc: 'VPN, internet, DNS, routing issues',
  },
  {
    icon: HardDrive,
    name: 'Hardware',
    color: 'bg-orange-100 text-orange-600',
    desc: 'Printers, monitors, peripherals',
  },
  {
    icon: Cpu,
    name: 'Software',
    color: 'bg-purple-100 text-purple-600',
    desc: 'Crashes, installs, license errors',
  },
  {
    icon: Lock,
    name: 'Access',
    color: 'bg-emerald-100 text-emerald-600',
    desc: 'Passwords, SSO, permissions',
  },
];

const examples = [
  {
    input: '"My laptop won\'t connect to the office WiFi after the Windows update"',
    output: {
      category: 'Network',
  ...
  {
    input: '"I can\'t log in to Jira with my SSO credentials since yesterday"',
    output: {
      category: 'Access',
      subcategory: 'SSO / Identity',
      confidence: '95%',
      team: 'IAM Team',
    },
  },
  {
    input: '"The projector in Conference Room B keeps disconnecting mid-call"',
    output: {
      category: 'Hardware',
      ...
      <section className='bg-gradient-to-br from-blue-50 via-white to-emerald-50 py-24 px-6 text-center'>
        <div className='max-w-4xl mx-auto'>
          ...
          <p className='text-xl text-gray-500 max-w-2xl mx-auto leading-relaxed'>
            Our AI reads the intent of every ticket and tags it with the right category,
            subcategory, and team — in under 200ms.
          </p>
        </div>
      </section>
      ...
      <section className='py-20 px-6 max-w-6xl mx-auto'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-4 gap-6'>
          {categories.map(({ icon: Icon, name, color, desc }) => (
            <div
              key={name}
              className='p-6 rounded-2xl border border-gray-100 bg-gray-50 hover:bg-white hover:shadow-lg hover:-translate-y-1 transition-all text-center'
            >
              <div
                className={`w-14 h-14 rounded-2xl mx-auto flex items-center justify-center mb-4 ${color}`}
              >
              ...
              >
                ...
                <div className='p-5 grid grid-cols-2 md:grid-cols-4 gap-4 bg-emerald-950/30'>
                  {Object.entries(ex.output).map(([key, val]) => (
                    <div key={key}>
                      <p className='text-[10px] text-emerald-500 uppercase tracking-widest font-bold mb-1'>
                        {key}
                      </p>
