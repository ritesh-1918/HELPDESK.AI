...
const priorities = [
  {
    ...
    desc: 'System outages, data breaches, total blockers. Responded to immediately.',
    examples: ['Server completely down', 'Security breach detected', 'All users locked out'],
  },
  {
    level: 'High',
    color: 'bg-orange-500',
    ...
    desc: 'Major feature broken, deadline-sensitive. Responded to within 1 hour.',
    examples: ['Production deploy failing', 'VPN down for team', 'Payment system error'],
  },
  {
    level: 'Medium',
    color: 'bg-yellow-500',
    ...
    desc: 'Partial functionality impacted. Responded to within 4 hours.',
    examples: ['Printer not working', 'Email delays', 'Slow performance'],
  },
  {
    level: 'Low',
    color: 'bg-green-500',
    ...
    desc: 'Minor inconvenience, cosmetic issues. Responded to within 24 hours.',
    examples: ['Wrong timezone on dashboard', 'Typo in email', 'Dark mode request'],
  },
];
    ...
    <div className='min-h-screen bg-white font-sans'>
      <nav className='sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100 px-6 py-4 flex items-center justify-between'>
        ...
        <button
          ...
          className='bg-emerald-900 text-white px-5 py-2 rounded-lg text-sm font-semibold hover:bg-emerald-800 transition-all'
        >
          Get Started Free
        </button>
      </nav>
      ...
      <section className='bg-gradient-to-br from-red-50 via-white to-orange-50 py-24 px-6 text-center'>
        <div className='max-w-4xl mx-auto'>
          ...
          <p className='text-xl text-gray-500 max-w-2xl mx-auto leading-relaxed'>
            Our AI reads emotional context, keywords, and impact signals to automatically assign the
            right priority — before any human sees it.
          </p>
        </div>
      </section>
      ...
      <section className='py-20 px-6 max-w-6xl mx-auto'>
        ...
        <div className='space-y-4'>
          {priorities.map(({ level, color, text, bg, desc, examples }) => (
            <div
              key={level}
              className={`rounded-2xl border p-6 ${bg} flex flex-col md:flex-row md:items-center gap-6`}
            >
              ...
              <div className='flex-1'>
                ...
                <div className='flex flex-wrap gap-2'>
                  {examples.map((ex) => (
                    <span
                      key={ex}
                      className='text-xs bg-white/80 border border-gray-200 text-gray-600 px-3 py-1 rounded-full font-medium'
                    >
                      {ex}
                    </span>
                  ))}
      ...
      <section className='bg-gray-950 py-24 px-6'>
        <div className='max-w-4xl mx-auto'>
          <h2 className='text-3xl font-bold text-white text-center mb-3'>
            Urgency Signals the AI Detects
          </h2>
      ...
      <section className='py-24 px-6 text-center bg-white'>
        ...
        <button
          onClick={() => navigate('/admin-signup')}
          className='inline-flex items-center gap-2 bg-emerald-900 text-white px-8 py-4 rounded-xl font-bold text-lg hover:bg-emerald-800 transition-all shadow-xl shadow-emerald-900/20'
        >
