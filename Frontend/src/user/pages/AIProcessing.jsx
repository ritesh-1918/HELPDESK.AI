      ...
      try {
        ...
        if (image_base64) {
          try {
            ...
            const fileName = `${user?.id || 'anon'}/${Date.now()}-${Math.random()
              .toString(36)
              .substring(7)}.${fileExt}`;
        ...
        if (buffer.trim()) {
          ...
          for (const line of lines) {
            ...
            try {
              ...
              console.error('Final buffer parse error', e, line);
            }
          }
        }
        ...
        try {
          ...
          if (
            aiResult.category &&
            (finalTicket.confidence < 0.6 ||
              finalTicket.category === 'Unknown' ||
              finalTicket.category === 'Access')
          ) {
