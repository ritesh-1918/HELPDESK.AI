    ...
    useEffect(() => {
        ...
        const generateInitialPlan = async () => {
            ...
            try {
                ...
                setMessages([{
                    role: 'bot',
                    text: `Hello! I've analyzed your request: "${aiTicket.summary}". I've put together a ${newSteps.length || sentences.length}-step troubleshooting plan to resolve this. How would you like to start?`,
                    timestamp: now
                }]);
        ...
        <div className="relative min-h-screen pt-24 pb-12 px-6 overflow-hidden">
            ...
            <div className="max-w-4xl mx-auto relative">
                ...
                <Card className="rounded-[2.5rem] border border-white/20 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] bg-white/70 backdrop-blur-3xl flex flex-col h-[820px] overflow-hidden transition-all duration-500 hover:shadow-[0_48px_80px_-20px_rgba(0,0,0,0.12)]">
                    ...
                    {steps.length > 0 && (
                        <div className="px-10 py-4 border-t border-white/40 bg-white/40 backdrop-blur-md">
                            <div className="flex items-center gap-2 mb-3">
                                <ListChecks size={16} className="text-emerald-600" />
                                <h3 className="text-[10px] font-black text-slate-700 uppercase tracking-[0.2em]">Troubleshooting Plan</h3>
                            </div>
                            <div className="space-y-2">
                                {steps.map((step) => (
                                    <div key={step.id} className="flex items-start gap-3">
                                        <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 text-[10px] font-black
                                            ${step.completed ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-500'}`}>
                                            {step.completed ? <CheckCircle2 size={12} /> : step.id}
                                        </div>
                                        <p className={`text-sm font-semibold leading-relaxed ${step.completed ? 'text-slate-400 line-through' : 'text-slate-700'}`}>
                                            {step.task}
                                        </p>
                                    </div>
                                ))}
