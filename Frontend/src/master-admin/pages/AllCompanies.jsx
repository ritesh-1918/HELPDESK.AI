    ...
    <div className='space-y-6'>
              ...
              filteredCompanies.map((company) => (
                <tr key={company.id} className='hover:bg-white/[0.01] transition-colors group'>
                  ...
                  <td className='px-6 py-4'>
                    ...
                    >
                      <span
                        className={`w-1 h-1 rounded-full ${company.status === 'active' ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`}
                      ></span>
                      {company.status}
                    </span>
                  </td>
      ...
      <div className='flex items-center justify-between px-6 py-4 bg-white/[0.01] border border-white/5 rounded-2xl'>
        <p className='text-xs font-medium text-slate-500'>
          Showing <span className='text-white'>{filteredCompanies.length}</span> of{' '}
          <span className='text-white'>{companies.length}</span> registered enterprises
        </p>
        <div className='flex gap-2'>
          <button
            disabled
            className='px-4 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs font-bold text-slate-500 cursor-not-allowed'
          >
            Previous
          </button>
          <button
