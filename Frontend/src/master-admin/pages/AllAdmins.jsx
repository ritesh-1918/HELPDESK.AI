      ...
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8'>
        <div>
          ...
          <p className='text-slate-400 text-sm mt-1'>
            Global directory of all company-level administrators.
          </p>
        </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6'>
        {loading ? (
          ...
          filteredAdmins.map((admin) => (
            <div
              key={admin.id}
              className='bg-white/[0.03] border border-white/5 rounded-2xl p-6 hover:border-white/10 transition-all group hover:-translate-y-1'
            >
                ...
                <div className='space-y-3 pt-4 border-t border-white/5'>
                  ...
                  <div className='grid grid-cols-2 gap-2'>
                    <div className='bg-white/5 rounded-lg p-2 border border-white/5'>
                      <p className='text-[8px] text-slate-500 uppercase tracking-widest font-bold mb-0.5'>
                        Industry
                      </p>
                      <p className='text-[10px] text-slate-300 font-bold truncate'>
                        {admin.industry || 'General'}
                      </p>
                    </div>
                    <div className='bg-white/5 rounded-lg p-2 border border-white/5'>
                      <p className='text-[8px] text-slate-500 uppercase tracking-widest font-bold mb-0.5'>
              ...
              <div className='flex gap-2'>
                <button
                  onClick={() =>
                    showToast(
                      'Admin protocol: Direct editing is restricted to Secure Vault. Launching proxy...',
                      'info'
                    )
                  }
                ...
                <button
                  onClick={() => {
                    if (
                      window.confirm(
                        `Are you sure you want to delete admin ${admin.full_name}? This will NOT delete their company but will remove their access.`
                      )
                    ) {
