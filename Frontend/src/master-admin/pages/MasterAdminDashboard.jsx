...
import { Users, Building2, Bell, ShieldCheck, Activity, Database, Zap } from 'lucide-react';
...
function MasterAdminDashboard() {
  ...
  const fetchStats = async () => {
    try {
      const [{ count: uCount }, { count: aCount }, { count: cCount }, { count: rCount }] =
        await Promise.all([
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: <Bell />,
      color: 'amber',
      growth: stats.pendingRequests > 0 ? 'Action Required' : 'Queue Clear',
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-slate-400 mt-1'>
            Real-time vitals and global statistics for HelpDesk.ai.
          </p>
        </div>
        ...
        {statCards.map((card, i) => (
          ...
          >
            <div className='flex items-start justify-between mb-4'>
              <div
                className={`w-12 h-12 rounded-xl bg-${card.color}-500/10 border border-${card.color}-500/20 flex items-center justify-center text-${card.color}-400 group-hover:scale-110 transition-transform`}
              >
                {React.cloneElement(card.icon, { className: 'w-6 h-6' })}
              </div>
              <span
                className={`text-[10px] font-black uppercase tracking-widest ${card.pulse ? 'text-amber-500 flex items-center gap-1.5' : 'text-slate-500'}`}
              >
                {card.growth}
              </span>
            </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
        ...
        <div className='bg-indigo-600/5 border border-indigo-500/10 rounded-3xl p-8 flex flex-col items-center justify-center text-center'>
          ...
          <button
            onClick={() =>
              showToast(
                'Read-Only: Platform Config Engine is restricted for security. Use CLI for overrides.',
                'warning'
              )
            }
