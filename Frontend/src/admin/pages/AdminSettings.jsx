...
import { Settings, Cpu, Inbox, Bell, Save, ShieldCheck } from 'lucide-react';
        ...
        <Card className='border-none shadow-2xl shadow-slate-200/40 rounded-[2rem] overflow-hidden bg-white'>
          ...
          <CardContent className='p-8 space-y-8'>
            ...
            <div className='space-y-4 pt-4 border-t border-slate-100'>
              <div className='flex justify-between items-center'>
                <label className='text-xs font-black text-slate-700 uppercase tracking-widest'>
                  Duplicate Detection (
                  <span className='text-indigo-600'>
                    {(settings.duplicateSensitivity * 100).toFixed(0)}%
                  </span>
                  )
                </label>
              </div>
            ...
            <div className='flex items-center justify-between pt-4 border-t border-slate-100'>
              ...
              >
                <div
                  className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all duration-300 shadow-md ${settings.enableAutoResolve ? 'right-1' : 'left-1'}`}
                ></div>
              </button>
            </div>
          </CardContent>
        ...
        <Card className='border-none shadow-2xl shadow-slate-200/40 rounded-[2rem] overflow-hidden bg-white'>
          ...
          <CardContent className='p-8 space-y-6'>
            ...
            <div className='flex items-center justify-between pb-4 border-b border-slate-100'>
              ...
              >
                <div
                  className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all duration-300 shadow-md ${settings.emailNotifications ? 'right-1' : 'left-1'}`}
                ></div>
              </button>
            </div>
            ...
            <div className='flex items-center justify-between'>
              ...
              >
                <div
                  className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all duration-300 shadow-md ${settings.adminAlerts ? 'right-1' : 'left-1'}`}
                ></div>
              </button>
            </div>
          </CardContent>
