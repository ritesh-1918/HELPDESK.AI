...
const AdminDashboard = () => {
  ...
  const aiSubsystems = useMemo(() => {
    ...
    const categorized = tickets.filter(
      (t) => t.category && t.category.toLowerCase() !== 'unassigned' && t.category !== 'Other'
    ).length;
    const prioritized = tickets.filter((t) => t.priority).length;
    return [
      {
        name: 'Classifier Engine',
        status: categorized > 0 ? 'Active' : 'Standby',
        latency: `${((categorized / totalCount) * 100).toFixed(0)}% Coverage`,
      },
      {
        name: 'Priority Routing',
        status: prioritized > 0 ? 'Active' : 'Standby',
        latency: `${((prioritized / totalCount) * 100).toFixed(0)}% Routed`,
      },
      {
        name: 'Semantic Analysis',
        status: tickets.length > 0 ? 'Active' : 'Standby',
        latency: `${tickets.length} Scanned`,
      },
  ...
  return (
    <div
      style={{ background: '#f8faf9', minHeight: '100vh', paddingBottom: '40px' }}
      className='space-y-10 -m-6 p-6 md:-m-10 md:p-10'
    >
      {/* Header */}
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        ...
        >
          <StatCard
            ...
            color='red'
            subtitle='Requires support agent'
          ...
          >
            <div className='space-y-4'>
              {aiSubsystems.map((sub, idx) => (
                <div
                  key={idx}
                  className='flex items-center justify-between p-4 rounded-2xl border border-gray-100 transition-all cursor-default hover:bg-white hover:border-green-100'
                  style={{ background: '#f8faf9' }}
                >
                  <div className='flex items-center gap-3'>
                    <div
                      style={{
                        background: aiIconMap[idx].bg,
                        color: aiIconMap[idx].color,
                        width: 36,
                        height: 36,
                        borderRadius: '10px',
                        display: 'flex',
