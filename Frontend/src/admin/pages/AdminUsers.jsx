...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (currentUser) {
        ...
        if (!freshProfile) {
          console.warn(
            '⚠️ Local profile missing from DB. Initiating Emergency Repair for:',
            currentUser.email
          );
      ...
      console.log(
        'Admin session synchronized. Company ID:',
        activeCompanyId,
        'Name:',
        activeCompanyName
      );
      ...
      let profileQuery = supabase.from('profiles').select('*').eq('status', 'pending_approval');

      // Inclusion Logic: Find any user that matches our ID OR our Name
      if (activeCompanyId && activeCompanyName) {
        profileQuery = profileQuery.or(
          `company_id.eq.${activeCompanyId},company.eq."${activeCompanyName}"`
        );
  ...
  const handleApproveUser = async (request) => {
    ...
    try {
      ...
      if (!targetCompanyId) {
        ...
        throw new Error(
          'Security Protocol Error: Your administrator account is not linked to a registered company. Please contact support.'
        );
      }
      ...
      await fetchUsers();
      showToast(
        `Protocol: Identity verified. ${request.user?.full_name} is now active.`,
        'success'
      );
    } catch (err) {
  ...
  const handleRejectUser = async (request) => {
    if (
      !window.confirm(`Are you sure you want to reject ${request.user?.full_name || 'this user'}?`)
    )
      return;
    setIsProcessing(request.id);
    try {
          ...
          <div className='w-full md:w-3/4 max-w-2xl bg-white md:rounded-[2.5rem] border-none shadow-2xl h-full flex flex-col animate-in slide-in-from-right-10 duration-300'>
            <div className='p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50 md:rounded-t-[2.5rem]'>
              ...
              <button
                ...
                className='text-slate-400 hover:text-slate-600 bg-white shadow-sm p-3 rounded-xl border border-slate-200 transition-colors'
              >
                <X size={24} />
              </button>
            </div>
            ...
            <div className='flex-1 overflow-y-auto p-8 space-y-8'>
              {/* User Info */}
              <div>
                <h4 className='text-[11px] font-black text-indigo-600 uppercase tracking-widest mb-4'>
                  Entity Metadata
                </h4>
                <div className='grid grid-cols-2 gap-4'>
                  <div className='p-4 bg-slate-50 rounded-2xl border border-slate-100'>
                    ...
                    <span className='text-[11px] font-mono font-black text-slate-800 break-all'>
                      {activeProfile.id}
                    </span>
                  </div>
                  <div className='p-4 bg-slate-50 rounded-2xl border border-slate-100'>
                    ...
                    <span
                      className={`text-xs font-black uppercase tracking-widest ${activeProfile.role === 'admin' ? 'text-indigo-600' : 'text-slate-600'}`}
                    >
                      {activeProfile.role}
                    </span>
                  </div>
            ...
            <div className='p-6 border-t border-slate-100 bg-slate-50 md:rounded-b-[2.5rem] flex gap-3'>
              <button
                onClick={() =>
                  handleUpdateRole(
                    activeProfile.id,
                    activeProfile.role === 'admin' ? 'user' : 'admin'
                  )
                }
            ...
            <div className='p-8 text-center space-y-6'>
              ...
              <div className='space-y-2'>
                ...
                <p className='text-sm text-slate-500 font-medium px-4 leading-relaxed'>
                  ...
                  <span className='font-black text-red-600 italic'>@{userToAction?.full_name}</span>
                  . This action remove their profile record from the system.
                </p>
              </div>
