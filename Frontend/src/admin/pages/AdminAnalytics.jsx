...
const AdminAnalytics = () => {
  ...
  const stats = useMemo(() => {
    ...
    const volumeTimeline = Object.keys(timeMap)
      .map((key) => ({ date: key, count: timeMap[key] }))
    ...
    const categoryData = Object.keys(catMap)
      .map((key) => ({ name: key, count: catMap[key] }))
    ...
    const teamData = Object.keys(teamMap)
      .map((key) => ({ name: key, value: teamMap[key] }))
    ...
    const resolutionData = Object.keys(statusMap).map((key) => ({
      name: key,
      value: statusMap[key],
    }));
    ...
    const liveFeed = tickets.slice(0, 10).map((t) => ({
      ...
      user:
        ...
        (t.user_id ? `User ${t.user_id.slice(0, 5)}` : 'Anonymous'),
  ...
  const aiStats = useMemo(() => {
    ...
    const accuracyRate = tickets.length
      ? (((tickets.length - corrected) / tickets.length) * 100).toFixed(1)
      : 0;
  ...
  if (loading)
    return (
      <div className='flex flex-col items-center justify-center min-h-[400px]'>
        ...
        <p className='text-slate-400 font-black uppercase tracking-widest italic text-center'>
  ...
  return (
    <div
      ...
      className='space-y-10 -m-6 p-6 md:-m-10 md:p-10 animate-in fade-in duration-700'
    >
      {/* Header section */}
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        ...
        <StatCard
          label='Average CSAT'
          value={
            aiStats.avgCsatScore ? (
              aiStats.avgCsatScore
            ) : (
              <span style={{ color: '#9ca3af', fontSize: '24px' }}>No data</span>
            )
          }
      ...
      <div className='grid grid-cols-1 lg:grid-cols-12 gap-10'>
          ...
          >
            ...
            <div className='h-[300px] w-full'>
              {stats.volumeTimeline.length > 0 ? (
                ...
                <div className='w-full h-full flex items-center justify-center text-slate-300 font-black uppercase text-[10px] italic tracking-widest'>
                  No activity data
                </div>
              )}
            </div>
                    ...
                    <PieChart>
                      ...
                      >
                        {aiStats.resolutionSplit.map((entry, index) => (
                          <Cell
                            key={index}
                            fill={index === 0 ? '#16a34a' : index === 1 ? '#3b82f6' : '#f59e0b'}
                            cornerRadius={4}
                          />
                        ))}
                      </Pie>
          ...
          <Card className='p-8 border-none shadow-xl shadow-slate-200/50 rounded-[2rem] bg-white'>
            <div className='mb-8'>
              ...
              <p className='text-[10px] text-slate-400 uppercase tracking-widest mt-1'>
                Categories with manual corrections
              </p>
            </div>
            <div className='h-[250px]'>
              {aiStats.misclassifiedCategories.length > 0 ? (
                <ResponsiveContainer width='100%' height='100%'>
                  <BarChart
                    data={aiStats.misclassifiedCategories}
                    margin={{ top: 0, right: 0, left: -20, bottom: 0 }}
                  >
                    <CartesianGrid strokeDasharray='3 3' vertical={false} stroke='#f1f5f9' />
                    <XAxis
                ...
                <div className='w-full h-full flex items-center justify-center flex-col gap-3'>
                  ...
                  <p className='text-slate-300 font-black uppercase text-[10px] italic tracking-widest text-center'>
                    AI classifications are performing optimally.
                  </p>
                </div>
              )}
            </div>
        ...
        <div className='lg:col-span-4'>
          ...
          >
            ...
            <div
              className='overflow-y-auto custom-scrollbar flex-1 p-6'
                    ...
                    >
                      <div className='flex flex-col items-center'>
                        ...
                        {idx !== stats.liveFeed.length - 1 && (
                          <div className='w-px h-full bg-gray-100 group-hover:bg-emerald-100 transition-colors my-1'></div>
                        )}
                      </div>
