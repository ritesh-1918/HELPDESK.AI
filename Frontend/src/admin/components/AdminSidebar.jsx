...
const AdminSidebar = ({ isMobile, onClose, isCollapsed, onToggleCollapse }) => {
  ...
  return (
    ...
    >
      {/* Logo Section */}
      <div
        className='p-6 border-b border-gray-50 flex items-center'
        style={{
          justifyContent: showLabels ? 'space-between' : 'center',
          padding: isCollapsed && !isMobile ? '24px 16px' : '24px 32px',
        }}
      >
        ...
        {navItems.map((item) => (
          <NavLink
            ...
            style={({ isActive }) => ({
              ...
              gap: '12px',
              borderRadius: '10px',
              padding: isCollapsed && !isMobile ? '10px' : '9px 14px',
          ...
          >
            ...
            {isCollapsed && !isMobile && (
              <div
                className='absolute left-full ml-3 bg-gray-900 text-white text-xs py-1.5 px-3 rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 whitespace-nowrap shadow-lg'
      ...
      <div className='p-4 border-t border-gray-50 space-y-1.5 pb-8 flex flex-col items-stretch'>
        ...
        >
          ...
          />
          {showLabels && (
            <span className='text-sm tracking-tight animate-in fade-in duration-300'>Settings</span>
          )}
        </NavLink>
        ...
        >
          ...
          {showLabels && (
            <span className='text-sm tracking-tight animate-in fade-in duration-300'>Logout</span>
          )}
        </button>
      </div>
    </aside>
