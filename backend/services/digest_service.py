            ...
            if status in ("resolved", "closed"):
                ...
                if created_str and end_str:
                    try:
                        ...
                        diff = (e_dt - c_dt).total_seconds() / 60.0
                        if diff >= 0:
...
def _load_email_template(custom_path: Optional[str] = None) -> str:
    template_path = Path(custom_path) if custom_path else TEMPLATE_DIR / "weekly_digest.html"
...
def send_digest_email(admin_email: str, stats: dict, ai_summary: str, custom_template_path: Optional[str] = None) -> bool:
    ...
    if sla_count > 0:
        sla_border_color = "#fca5a5"
        sla_text_color = "#dc2626"
    else:
        sla_border_color = "#e5e7eb"
        sla_text_color = "#111827"
    ...
    template_str = _load_email_template(custom_template_path)
