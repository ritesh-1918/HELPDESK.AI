...
_SPEED_REFERENCE_HOURS = 4.0
_VOLUME_REFERENCE = 20.0


def compute_performance_score(metrics: dict, custom_weights: dict | None = None) -> float:
    if metrics.get("total_tickets", 0) == 0:
        return 0.0

    weights = custom_weights or {"resolution": 40.0, "sla": 30.0, "speed": 20.0, "volume": 10.0}

    resolution_score = metrics.get("resolution_rate", 0.0) * weights.get("resolution", 40.0)
    sla_score = metrics.get("sla_compliance_rate", 0.0) * weights.get("sla", 30.0)

    avg_hours = metrics.get("avg_resolution_hours")
    if avg_hours is not None and avg_hours >= 0:
        speed_score = weights.get("speed", 20.0) * math.exp(-avg_hours / (_SPEED_REFERENCE_HOURS * 5))
        speed_score = max(0.0, min(weights.get("speed", 20.0), speed_score))
    else:
        speed_score = weights.get("speed", 20.0) / 2.0

    volume = metrics.get("total_tickets", 0)
    volume_score = min(weights.get("volume", 10.0), (volume / _VOLUME_REFERENCE) * weights.get("volume", 10.0))
