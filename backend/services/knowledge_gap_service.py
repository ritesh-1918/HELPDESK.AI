...
class KnowledgeGapService:
    ...
    async def detect_gaps(self, company_id: str) -> None:
        ...
        for i in range(len(vectors)):
            ...
            if norm_a > 0:
                sims = np.dot(X, vectors[i]) / (norms * norm_a)
            else:
                sims = np.zeros(len(vectors))
                
            neighbors = np.where(sims >= 0.85)[0].tolist()
    ...
    async def convert_ticket_to_article(self, ticket_id: str, company_id: str) -> dict:
        ...
        if not self.gemini_service or not getattr(self.gemini_service, "_initialized", False):
            ...
            prompt = (
                f"You are a professional technical writer. Transform this resolved ticket into a professional knowledge base article.\n"
