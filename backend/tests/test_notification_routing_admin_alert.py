...
Coverage for issue: Add unit tests for notification_routing
    should_send_admin_alert method
"""
...
class TestShouldSendAdminAlert:
    def test_sends_when_admin_alerts_enabled(self, middleware):
        ...
        assert middleware.should_send_admin_alert("company-abc") is True

    def test_blocks_when_admin_alerts_disabled(self, middleware):
        middleware.supabase.table.return_value.select.return_value.eq.return_value.single.return_value.execute.return_value = mock.MagicMock(
            data={"email_notifications": True, "admin_alerts": False, "digest_frequency": "daily"}
        )

        assert middleware.should_send_admin_alert("company-abc") is False
    ...
    def test_cache_reuse_skips_supabase(self, middleware):
        ...
        middleware.should_send_admin_alert("company-cache")
        middleware.should_send_admin_alert("company-cache")
    ...
    def test_invalidate_cache_refetches(self, middleware):
        ...
        middleware.should_send_admin_alert("company-cache2")
